...
import { getContactById } from "../../../services/contact.service.ts";
import { getAddresses } from "../../../services/address.service.ts";
import type { IAddress } from "../../../types/address.ts";
...
const ContactDetail = () => {
  ...
  const { data: contact } = useQuery({
    queryKey: ["contact", id],
    queryFn: async () => await getContactById(String(id)),
  });

  const { data: addreses } = useQuery({
    queryKey: ["adresses", id],
    queryFn: async () => await getAddresses(String(id)),
  });
        ...
        <div className="p-8">
          ...
          <div className="mb-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              ...
              {addreses?.map((address: IAddress, index: number) => (
                <div
                  key={address.id}
